...
class Application:
    def __init__(self):
        ...
        self.ui_manager = Ui_MainWindowViewerReports()
    ...
    def fill_report_table_date(self):
        start_date, finish_date = self.combo_box_manager.get_selected_date(
            self.combo_box_manager.get_selected_month(self.ui_manager.comboBox_month),
            self.combo_box_manager.get_selected_year(self.ui_manager.comboBox_year)
        )
        ...
        self.table_manager.fill_order_table(
            self.ui_manager.tableView_date,
            orders,
            self.db_manager.get_client_dict(),
            self.db_manager.get_personnel_dict()
        )
        self.table_manager.adjust_columns(self.ui_manager.tableView_date)
    ...
    def fill_report_table_emp(self):
        orders = self.db_manager.get_orders_by_personnel(
            self.combo_box_manager.get_selected_employee_code(
                self.ui_manager.comboBox_emp
            )
        )
        self.table_manager.fill_order_table(
            self.ui_manager.tableView_emp,
            orders,
            self.db_manager.get_client_dict(),
            self.db_manager.get_personnel_dict()
        )
        self.table_manager.adjust_columns(self.ui_manager.tableView_emp)